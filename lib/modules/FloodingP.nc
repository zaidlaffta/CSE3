...
implementation {
    ...
    // New function: Reset the flooded packet counter
    void resetFloodedPacketCount() {
        floodedPacketCount = 0;
        dbg(FLOODING_CHANNEL, "Flooded packet counter has been reset to 0\n");
    }

    // New function: Log the current sequence number
    void logCurrentSequenceNumber() {
        dbg(FLOODING_CHANNEL, "Current sequence number is %d\n", sequenceNum);
    }
