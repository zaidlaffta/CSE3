...
{
	uses interface Timer<TMilli> as rebuildLinkStateRoutingTimer;
	uses interface Hashmap<destination_node> as LinkStateRouting;
...
implementation
{
	uint16_t LinkStateRoutingFloodSeq = 0;
	...
	command void LinkStateRouting.start()
	{
		...
		call LinkStateRouting.reset();
		receivedLinks = 0;

		call NeighborDiscovery.start();

		call rebuildLinkStateRoutingTimer.startPeriodic(ROUTING_TABLE_REBUILD_DELAY);
	}

	command uint16_t LinkStateRouting.getNextHop(uint16_t dest)
	{
		if (call LinkStateRouting.contains(dest))
		{
			return (call LinkStateRouting.get(dest)).nextHop;
		}
		else
		{
			return ~0;
	 ...
	 */
	event void rebuildLinkStateRoutingTimer.fired()
	{
		...
		packet.TTL = MAX_TTL;
		packet.seq = LinkStateRoutingFloodSeq++;
		...
		if (LinkStateRoutingFloodSeq % 4 != 0)
		{
		...
		while (call unvisitedNodes.size() > 0 && fail_safe-- > 0)
		{
			...
			call LinkStateRouting.insert(currentNodeId, currentNode);
	...
	command message_t *LinkStateRouting.receive(message_t * raw_msg, void *payload, uint8_t len)
	{
		LinkState *lsa;
		uint16_t i, j;
		// uint8_t reliability;
	...
	{
		uint32_t i, j;
		uint16_t nodeCount = call LinkStateRouting.size();
		uint16_t *nodeIDs = call LinkStateRouting.getKeys();
		...
		for (i = 0; i <= nodeCount + 1; i++)
		{
			for (j = 0; j < nodeCount; j++)
			{
				...
				destination_node node = call LinkStateRouting.get(dest);
